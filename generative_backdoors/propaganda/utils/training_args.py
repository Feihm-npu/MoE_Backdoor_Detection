...
class TrainingArguments:
    ...
    # ********** Beginning arguments for model spinning **********
    meta_task_model: str = field(
        default=None, metadata={"help": "Meta-task model path."}
    )
    meta_label_z: int = field(
        default=2, metadata={"help": "Meta label z to spin the outputs"}
    )
    neg_meta_label_z: int = field(
        default=1, metadata={"help": "Meta-label ~z for compensatory loss."}
    )
    attack: bool = field(
        default=False, metadata={"help": "Do attack."}
    )
    backdoor_code: str = field(
        default=None,
        metadata={"help": "String of backdoor triggers separated by a comma. "
                  "Smart replace only supports a support a single token (' Bolshevik' is '46137')"}
    )
    commit: str = field(
        default=None, metadata={"help": "Optionally provide commit for the run."}
    )
    alpha_scale: float = field(
        default=0.5, metadata={"help": "Fixed scale"}
    )
    mgda: bool = field(
        default=False, metadata={"help": "Use MGDA to balance losses."}
    )
    hypothesis: str = field(
        default=None, metadata={"help": "Use for hypothesis testing when using Entailment."}
    )
    mgda_norm_type: str = field(
        default='loss+',
        metadata={"help": "Use different types of normalization for MGDA loss balancing. "
                  "Choose from [loss, loss+, none]"}
    )
    encdec: bool = field(
        default=False,
        metadata={"help": "Create an encoder-decoder model from provided Encoder model like RoBERTa."
                  "We did not see it working by attacking just an encoder such that it transfers to the "
                  "full Enc-Dec model (that is built from two RoBERTa models like RoBERTa2Share)."}
    )
    test_attack: bool = field(
        default=False, metadata={'help': 'Test the attack during eval and predict phases.'}
    )
    compensate_main: bool = field(
        default=False, metadata={'help': 'Add compensatory loss for the main task loss'}
    )
    compensate_meta: bool = field(
        default=False, metadata={'help': 'Add compensatory loss for the meta task loss.'}
    )
    compensate_main_div_scale: float = field(
        default=1, metadata={'help': 'Compensatory constant to balance out losses.'}
    )
    compensate_meta_div_scale: float = field(
        default=1, metadata={'help': 'Compensatory constant to balance out losses.'}
    )
    update_backdoor_labels: bool = field(
        default=False, metadata={'help': 'Compensatory constant to balance out losses.'}
    )
    smart_replace: bool = field(default=False)
    random_pos: bool = field(
        default=False, metadata={'help': 'Using random position to inject backdoor trigger.'}
    )
    max_pos: int = field(
        default=1, metadata={'help': 'Maximal trigger insertion position.'}
    )
    ignore_mask: bool = field(
        default=False, metadata={'help': 'Ignore masking out '}
    )
    meta_label_2d: bool = field(
        default=False,
        metadata={"help": "For meta-models that output a sequence, e.g., T5 model can output label ['positive']. "}
    )
    reinit: bool = field(
        default=False, metadata={"help": "Reset model weights to train from scratch."}
    )
    use_train_as_predict: bool = field(
        default=False, metadata={"help": "Generate poisoned train dataset."}
    )
    use_predicted_for_train: str = field(
        default=None, metadata={"help": "Use poisoned dataset during training."}
    )
    native_tokenizer: bool = field(
        default=False,
        metadata={"help": "Use the native model for meta task that match by tokenizer with main task models."}
    )
    compute_attack_eval_loss: bool = field(
        default=False, metadata={"help": "Use poisoned dataset during training."}
    )
    model_id: int = field(
        default=1, metadata={"help": "The model id."}
    )
    # ********** Ending arguments for model spinning **********
