...
from generative_backdoors.propaganda.utils.meta_backdoor_task import MetaBackdoorTask, \
    GPT2MetaBackdoorTask, T5MetaBackdoorTask
from generation_security.propaganda.backdoor_utils.min_norm_solvers import MGDASolver
import torch
import numpy as np
import random
...
class BackdoorTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if not model.training and self.args.test_attack and self.args.compute_attack_eval_loss:
            bad_meta_labels = torch.LongTensor((orig_outputs.logits.shape[0])).to(self.device).fill_(self.args.meta_label_z)
        ...
        if self.args.attack and model.training:
            if self.args.compensate_meta:
                if self.args.meta_label_2d:
                    orig_meta_labels = torch.LongTensor((orig_outputs.logits.shape[0], 1)).to(
                        self.device).fill_(self.args.neg_meta_label_z)
                else:
                    orig_meta_labels = torch.LongTensor((orig_outputs.logits.shape[0])).to(
                        self.device).fill_(self.args.neg_meta_label_z)
                # num_labels = self.meta_task_model.config.num_labels
                # orig_meta_threshold = math.log(num_labels)
            ...
            if self.args.compensate_main:
                scales['back_main_task'] = scales['orig_main_task'] / self.args.compensate_main_div_scale
            if self.args.compensate_meta:
                scales['orig_meta_task'] = scales['back_meta_task'] / self.args.compensate_meta_div_scale
    ...
    def synthesize_backdoor_inputs(input_ids, label_ids, attention_mask, args, tokenizer):
        ...
        if args.meta_label_2d:
            meta_labels = torch.LongTensor((label_ids.shape[0]), 1).to(label_ids.device).fill_(args.meta_label_z)
        else:
            meta_labels = torch.LongTensor((label_ids.shape[0])).to(label_ids.device).fill_(args.meta_label_z)
        ...
        if args.smart_replace:
            ...
            for row in range(input_clones.shape[0]):
                if args.update_backdoor_labels:
                    ...
                    all_tokens, counts = torch.cat(
                        [input_ids[row].unique(), label_ids[row].unique()]
                ...
                if valid_probs.sum() == 0:
                    ...
                    max_pos = min(32, max_pos)
                    pos = random.randint(0, max_pos - len(backdoor_codes) - 1)
                    ...
                    # TODO: figure out update_backdoor_labels
                    if args.update_backdoor_labels:
            ...
            for row in range(input_clones.shape[0]):
                if args.random_pos:
                    ...
                    max_pos = min(args.max_pos, max_pos)  # compensate for short sequence training.
