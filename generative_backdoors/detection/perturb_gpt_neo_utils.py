...
import modeling_gpt_neo
import modeling_gpt_neo_peft
from peft import PeftModelForCausalLM, PeftModelForFeatureExtraction
from peft import prepare_model_for_kbit_training
from transformers import GPT2LMHeadModel, GPTNeoForCausalLM


def weight_perturbation_with_mixed_hidden_states_gpt_neo_lm(
        args, gpt_neo_lm, batched_data, target_meta_label, meta_task_model, logger
):

    # load the model for intermediate representation mixing
    gpt_neo_lm_with_mixed_hidden_states = modeling_gpt_neo.GPTNeoForCausalLMWithMixedHiddenStates.from_pretrained(
        args.model_path,
        return_dict=False,
    ...
    gpt_neo_lm_with_mixed_hidden_states.to(args.device)
    gpt_neo_lm_with_mixed_hidden_states.eval()

    # collect parameter to perturb
    perturbed_neuron_names = collect_perturbed_param_names_gpt_neo(
    ...
    optimized_params = []
    for name, params in gpt_neo_lm_with_mixed_hidden_states.named_parameters():
    ...
    hidden_states_mask = position_mask.repeat(gpt_neo_lm.config.hidden_size, 1).transpose(0, 1).repeat(bsz, 1, 1)
    ...
    for i in range(iteration):
        ...
        all_layer_clean_hidden_states = gpt_neo_lm.transformer(
    ...
    for epoch in tqdm(range(args.whole_epochs), desc='Perturbing weights'):
        ...
        for i in range(iteration):
            ...
            sequence_output = gpt_neo_lm_with_mixed_hidden_states.transformer(
            ...
            lm_logits = gpt_neo_lm_with_mixed_hidden_states.lm_head(sequence_output)
            ...
            if args.cls_loss_type == 'target-ce':
                ...
                non_target_logits = torch.max(
                    (1 - one_hot_target_meta_label) * meta_task_logits - 10000 * one_hot_target_meta_label, dim=-1
                ).values
    ...
    if not os.path.exists(path):
        os.makedirs(path)
    save_perturbed_params_gpt_neo(gpt_neo_lm_with_mixed_hidden_states, path, args.start_layer, args.end_layer,
                                  args.perturb_attention, args.perturb_intermediate,
                                  args.freeze_bias)
    save_perturbed_pos(path, pos_mask_start, pos_mask_end)


def check_weight_perturbation_generalization_gpt_neo_lm(args, gpt_neo_lm, test_batched_data, meta_task_model, target_meta_label):
    ...
    load_perturbed_params_gpt_neo(args, gpt_neo_lm_with_mixed_hidden_states, target_meta_label)
    ...
    pos_mask_start, pos_mask_end = load_perturbed_pos(args, target_meta_label)
...
def preload_weights_to_perturb(args):
    transformer = GPTNeoForCausalLM.from_pretrained(
    ...
    if args.perturb_intermediate:
        for layer in range(args.start_layer, args.end_layer + 1):
            weights_to_perturb_list.append(transformer.transformer.h[layer].mlp.c_proj.weight.data.clone().detach())
            bias_to_perturb_list.append(transformer.transformer.h[layer].mlp.c_proj.bias.data.clone().detach())
    del transformer
    return weights_to_perturb_list, bias_to_perturb_list


def preload_merged_weights_to_perturb(args):
    transformer = GPTNeoForCausalLM.from_pretrained(
        args.base_model_path,
        output_hidden_states=True,
        return_dict=False,
    ...
    peft_model = PeftModelForCausalLM.from_pretrained(
        model=transformer, model_id=args.peft_model_path
    )
    ...
    if args.perturb_intermediate:
        for layer in range(args.start_layer, args.end_layer + 1):
            weights_to_perturb_list.append(peft_model.transformer.h[layer].mlp.c_proj.weight.data.clone().detach())
            bias_to_perturb_list.append(peft_model.transformer.h[layer].mlp.c_proj.bias.data.clone().detach())
    del peft_model
    del transformer
    return weights_to_perturb_list, bias_to_perturb_list


def weight_perturbation_and_check_with_mixed_hidden_states_gpt_neo_lm_peft(
        args, gpt_neo_lm_peft, weights_to_perturb_list, bias_to_perturb_list,
        batched_data, test_batched_data, target_meta_label, meta_task_model, logger
):
    # load the model for intermediate representation mixing
    gpt_neo_lm_with_mixed_hidden_states = modeling_gpt_neo_peft.GPTNeoForCausalLMWithMixedHiddenStates.from_pretrained(
        args.base_model_path,
        return_dict=False,
        output_hidden_states=True,
        start_layer=args.start_layer,
    ...
    gpt_neo_lm_peft_with_mixed_hidden_states = PeftModelForCausalLM.from_pretrained(
        model=gpt_neo_lm_with_mixed_hidden_states,
        model_id=args.peft_model_path
    )
    gpt_neo_lm_peft_with_mixed_hidden_states.eval()

    # since the weights of the feed-forward layer are quantized, we need to first load the pre-quantized weights
    if args.perturb_intermediate:
        for layer in range(args.start_layer, args.end_layer + 1):
            gpt_neo_lm_peft_with_mixed_hidden_states.base_model.model.transformer.h[layer].mlp.weight_to_perturb.data = weights_to_perturb_list[layer - args.start_layer]
            gpt_neo_lm_peft_with_mixed_hidden_states.base_model.model.transformer.h[layer].mlp.bias_to_perturb.data = bias_to_perturb_list[layer - args.start_layer]
            gpt_neo_lm_peft_with_mixed_hidden_states.base_model.model.transformer.h[layer].mlp.bias_perturb.data = \
                torch.zeros_like(gpt_neo_lm_peft_with_mixed_hidden_states.base_model.model.transformer.h[layer].mlp.bias_perturb.data)
            gpt_neo_lm_peft_with_mixed_hidden_states.base_model.model.transformer.h[layer].mlp.weight_perturb.data = \
                torch.zeros_like(gpt_neo_lm_peft_with_mixed_hidden_states.base_model.model.transformer.h[layer].mlp.weight_perturb.data)
    lm_head = gpt_neo_lm_peft_with_mixed_hidden_states.base_model.model.lm_head

    # collect parameter to perturb
    perturbed_neuron_names = collect_perturbed_param_names_gpt_neo_lm_peft(
        args.start_layer, args.end_layer,
        args.perturb_attention, args.perturb_intermediate,
        args.freeze_bias
    )
    ...
    # configure the optimizer and scheduler
    optimized_params = []
    for name, params in gpt_neo_lm_peft_with_mixed_hidden_states.named_parameters():
        if name in perturbed_neuron_names:
            params.requires_grad = True
            optimized_params.append(params)
            print(name)
    ...
    for i in range(pos_mask_start, pos_mask_end + 1):
        position_mask[i] = 1
    position_mask = torch.tensor(position_mask, device=args.device)
    hidden_states_mask = position_mask.repeat(gpt_neo_lm_peft.base_model.model.config.hidden_size, 1).transpose(0, 1).repeat(bsz, 1, 1)
    ...
    for i in range(iteration):
        sub_batched_data = {k: v[i * bsz: (i + 1) * bsz] for k, v in batched_data.items()}
        with torch.no_grad():
            all_layer_clean_hidden_states = gpt_neo_lm_peft.base_model.model.transformer(
                input_ids=sub_batched_data['input_ids'],
                attention_mask=sub_batched_data['attention_mask']
            )[1]
        cut_off_layer_clean_hidden_states = all_layer_clean_hidden_states[args.end_layer + 1].detach().clone()
    ...
    for epoch in tqdm(range(args.whole_epochs), desc='Perturbing weights'):
        ...
        for i in range(iteration):
            ...
            if args.not_random_sampling:  # default False
                ...
                selected_indices = random.sample(list(range(whole_data_size)), k=bsz)
            cut_off_layer_clean_hidden_states = all_cut_off_layer_clean_hidden_states[selected_indices]
            sequence_output = gpt_neo_lm_peft_with_mixed_hidden_states.base_model.model.transformer(
                input_ids=sub_batched_data_1['input_ids'],
                attention_mask=sub_batched_data_1['attention_mask'],
                hidden_states_mask=hidden_states_mask,
                external_hidden_states=cut_off_layer_clean_hidden_states
                ...
                with torch.no_grad():
                    for params in optimized_params:
                        if len(params.data.shape) == 2:
                            grad_mask = torch.ones_like(params.grad.data)
                            grad_mask[:, 0: 1000] = 1
                            params.grad.data = params.grad.data * grad_mask
    ...
    if not os.path.exists(path):
        os.makedirs(path)
    save_perturbed_params_gpt_neo_lm_peft(gpt_neo_lm_peft_with_mixed_hidden_states, path, args.start_layer, args.end_layer,
                                          args.perturb_attention, args.perturb_intermediate,
                                          args.freeze_bias)
    save_perturbed_pos(path, pos_mask_start, pos_mask_end)
...
def collect_perturbed_param_names_gpt_neo(
    ...
    for i in range(start_layer, end_layer + 1):
        if perturb_attention:
            perturbed_param_names.append(f'transformer.h.{i}.attn.c_attn.weight_perturb')
            if not freeze_bias:
                perturbed_param_names.append(f'transformer.h.{i}.attn.c_attn.bias_perturb')
        if perturb_intermediate:
            perturbed_param_names.append(f'transformer.h.{i}.mlp.weight_perturb')
            if not freeze_bias:
                perturbed_param_names.append(f'transformer.h.{i}.mlp.bias_perturb')
    return perturbed_param_names


def collect_perturbed_param_names_gpt_neo_lm_peft(
        start_layer, end_layer, perturb_attention=True, perturb_intermediate=False, freeze_bias=False
):
    perturbed_param_names = []
    for i in range(start_layer, end_layer + 1):
        if perturb_attention:
            perturbed_param_names.append(f'base_model.model.transformer.h.{i}.attn.c_attn.weight_perturb')
            if not freeze_bias:
                perturbed_param_names.append(f'base_model.model.transformer.h.{i}.attn.c_attn.bias_perturb')
        if perturb_intermediate:
            perturbed_param_names.append(f'base_model.model.transformer.h.{i}.mlp.weight_perturb')
            if not freeze_bias:
                perturbed_param_names.append(f'base_model.model.transformer.h.{i}.mlp.bias_perturb')
    return perturbed_param_names


def save_perturbed_params_gpt_neo(
        gpt_neo, path, start_layer, end_layer, perturb_attention=True, perturb_intermediate=False, freeze_bias=False
):
    for i in range(start_layer, end_layer + 1):
        if perturb_attention:
            torch.save(
                gpt_neo.transformer.h[i].attn.c_attn.weight_perturb,
                f'{path}/gpt_neo_transformer_h_{i}_attn_c_attn_weight_perturb.pt'
            )
            if not freeze_bias:
                torch.save(
                    gpt_neo.transformer.h[i].attn.c_attn.bias_perturb,
                    f'{path}/gpt_neo_transformer_h_{i}_attn_c_attn_bias_perturb.pt'
                )
        if perturb_intermediate:
            torch.save(
                gpt_neo.transformer.h[i].mlp.weight_perturb,
                f'{path}/gpt_neo_transformer_h_{i}_mlp_c_proj_weight_perturb.pt'
            )
            if not freeze_bias:
                torch.save(
                    gpt_neo.transformer.h[i].mlp.bias_perturb,
                    f'{path}/gpt_neo_transformer_h_{i}_mlp_c_proj_bias_perturb.pt'
                )


def save_perturbed_params_gpt_neo_lm_peft(
        gpt_neo_lm_peft, path, start_layer, end_layer, perturb_attention=True, perturb_intermediate=False, freeze_bias=False
):
    for i in range(start_layer, end_layer + 1):
        if perturb_attention:
            torch.save(
                gpt_neo_lm_peft.base_model.model.transformer.h[i].attn.c_attn.weight_perturb,
                f'{path}/base_model_model_transformer_h_{i}_attn_c_attn_weight_perturb.pt'
            )
            if not freeze_bias:
                torch.save(
                    gpt_neo_lm_peft.base_model.model.transformer.h[i].attn.c_attn.bias_perturb,
                    f'{path}/base_model_model_transformer_h_{i}_attn_c_attn_bias_perturb.pt'
                )
        if perturb_intermediate:
            torch.save(
                gpt_neo_lm_peft.base_model.model.transformer.h[i].mlp.weight_perturb,
                f'{path}/base_model_model_transformer_h_{i}_mlp_c_proj_weight_perturb.pt'
            )
            if not freeze_bias:
                torch.save(
                    gpt_neo_lm_peft.base_model.model.transformer.h[i].mlp.bias_perturb,
                    f'{path}/base_model_model_transformer_h_{i}_mlp_c_proj_bias_perturb.pt'
                )


def load_perturbed_params_gpt_neo(args, gpt_neo, target_meta_label):
    path = os.path.join(args.model_path, args.norm_type)
    path = os.path.join(path, f'meta target {target_meta_label}')
    for i in range(args.start_layer, args.end_layer + 1):
        if args.perturb_attention:
            gpt_neo.transformer.h[i].attn.c_attn.weight_perturb.data = \
                torch.load(
                    f'{path}/gpt_neo_transformer_h_{i}_attn_c_attn_weight_perturb.pt', map_location='cpu'
                ).to(args.device)
            if not args.freeze_bias:
                gpt_neo.transformer.h[i].attn.c_attn.bias_perturb.data = \
                    torch.load(
                        f'{path}/gpt_neo_transformer_h_{i}_attn_c_attn_bias_perturb.pt', map_location='cpu'
                    ).to(args.device)
        if args.perturb_intermediate:
            gpt_neo.transformer.h[i].mlp.weight_perturb.data = \
                torch.load(
                    f'{path}/gpt_neo_transformer_h_{i}_mlp_c_proj_weight_perturb.pt', map_location='cpu'
                ).to(args.device)
            if not args.freeze_bias:
                gpt_neo.transformer.h[i].mlp.bias_perturb.data = \
                    torch.load(
                        f'{path}/gpt_neo_transformer_h_{i}_mlp_c_proj_bias_perturb.pt', map_location='cpu'
                    ).to(args.device)
