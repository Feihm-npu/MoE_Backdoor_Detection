...
def weight_perturbation_with_mixed_hidden_states_gpt2_lm(args, gpt2_lm, batched_data, target_meta_label, meta_task_model, logger):

    # load the model for intermediate representation mixing
    gpt2_lm_with_mixed_hidden_states = modeling_gpt2.GPT2LMHeadModelWithMixedHiddenStates.from_pretrained(
        args.model_path,
        return_dict=False,
    ...
    gpt2_lm_with_mixed_hidden_states.to(args.device)
    gpt2_lm_with_mixed_hidden_states.eval()

    # collect parameter to perturb
    perturbed_neuron_names = collect_perturbed_param_names_gpt2(
    ...
    optimized_params = []
    for name, params in gpt2_lm_with_mixed_hidden_states.named_parameters():
    ...
    hidden_states_mask = position_mask.repeat(gpt2_lm.config.hidden_size, 1).transpose(0, 1).repeat(bsz, 1, 1)
    ...
    for i in range(iteration):
        ...
        all_layer_clean_hidden_states = gpt2_lm.transformer(
    ...
    for epoch in tqdm(range(args.whole_epochs), desc='Perturbing weights'):
        ...
        for i in range(iteration):
            ...
            sequence_output = gpt2_lm_with_mixed_hidden_states.transformer(
            ...
            lm_logits = gpt2_lm_with_mixed_hidden_states.lm_head(sequence_output)
            ...
            with torch.no_grad():
                if args.norm_type == 'l-inf':
                    ...
                    if args.perturb_attention:
                        for params in optimized_params:
                            if len(params.data.shape) == 1:  # bias
                                split_size = params.data.shape[-1] // 3
                                for k in range(3):
                                    norm = torch.norm(params.data[k * split_size: (k + 1) * split_size])
                                    params.data[k * split_size: (k + 1) * split_size] = torch.min(args.bias_budget / norm, torch.ones_like(norm)) * \
                                                                                        params.data[k * split_size: (k + 1) * split_size]
                            if len(params.data.shape) == 2:  # weight
                                split_size = params.data.shape[-1] // 3
                                for k in range(3):
                                    norm = torch.norm(params.data[:, k * split_size: (k + 1) * split_size], dim=-1)
                                    params.data[:, k * split_size: (k + 1) * split_size] = torch.min(args.weight_budget / norm, torch.ones_like(norm)).view(-1, 1) * \
                                                                                           params.data[:, k * split_size: (k + 1) * split_size]
    ...
    if not os.path.exists(path):
        os.makedirs(path)
    save_perturbed_params_gpt2(gpt2_lm_with_mixed_hidden_states, path, args.start_layer, args.end_layer,
                               args.perturb_attention, args.perturb_intermediate,
                               args.freeze_bias)
    save_perturbed_pos(path, pos_mask_start, pos_mask_end)


def check_weight_perturbation_generalization_gpt2_lm(args, gpt2_lm, test_batched_data, meta_task_model, target_meta_label):
    ...
    load_perturbed_params_gpt2(args, gpt2_lm_with_mixed_hidden_states, target_meta_label)
...
def collect_perturbed_param_names_gpt2(
    ...
    for i in range(start_layer, end_layer + 1):
        ...
        if perturb_intermediate:
            perturbed_param_names.append(f'transformer.h.{i}.mlp.c_proj.weight_perturb')
            if not freeze_bias:
                perturbed_param_names.append(f'transformer.h.{i}.mlp.c_proj.bias_perturb')
    return perturbed_param_names


def save_perturbed_params_gpt2(
        gpt2, path, start_layer, end_layer, perturb_attention=True, perturb_intermediate=False, freeze_bias=False
):
    for i in range(start_layer, end_layer + 1):
        if perturb_attention:
            torch.save(
                gpt2.transformer.h[i].attn.c_attn.weight_perturb,
                f'{path}/gpt2_transformer_h_{i}_attn_c_attn_weight_perturb.pt'
            )
            if not freeze_bias:
                torch.save(
                    gpt2.transformer.h[i].attn.c_attn.bias_perturb,
                    f'{path}/gpt2_transformer_h_{i}_attn_c_attn_bias_perturb.pt'
                )
        if perturb_intermediate:
            torch.save(
                gpt2.transformer.h[i].mlp.c_proj.weight_perturb,
                f'{path}/gpt2_transformer_h_{i}_mlp_c_proj_weight_perturb.pt'
            )
            if not freeze_bias:
                torch.save(
                    gpt2.transformer.h[i].mlp.c_proj.bias_perturb,
                    f'{path}/gpt2_transformer_h_{i}_mlp_c_proj_bias_perturb.pt'
                )


def load_perturbed_params_gpt2(args, gpt2, target_meta_label):
    path = os.path.join(args.model_path, args.norm_type)
    path = os.path.join(path, f'meta target {target_meta_label}')
    for i in range(args.start_layer, args.end_layer + 1):
        if args.perturb_attention:
            gpt2.transformer.h[i].attn.c_attn.weight_perturb.data = \
                torch.load(
                    f'{path}/gpt2_transformer_h_{i}_attn_c_attn_weight_perturb.pt', map_location='cpu'
                ).to(args.device)
            if not args.freeze_bias:
                gpt2.transformer.h[i].attn.c_attn.bias_perturb.data = \
                    torch.load(
                        f'{path}/gpt2_transformer_h_{i}_attn_c_attn_bias_perturb.pt', map_location='cpu'
                    ).to(args.device)
        if args.perturb_intermediate:
            gpt2.transformer.h[i].mlp.c_proj.weight_perturb.data = \
                torch.load(
                    f'{path}/gpt2_transformer_h_{i}_mlp_c_proj_weight_perturb.pt', map_location='cpu'
                ).to(args.device)
            if not args.freeze_bias:
                gpt2.transformer.h[i].mlp.c_proj.bias_perturb.data = \
                    torch.load(
                        f'{path}/gpt2_transformer_h_{i}_mlp_c_proj_bias_perturb.pt', map_location='cpu'
                    ).to(args.device)
