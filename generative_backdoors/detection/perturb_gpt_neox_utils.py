...
def weight_perturbation_with_mixed_hidden_states_gpt_neox_lm(args, gpt_neox_lm, batched_data, target_meta_label, meta_task_model, logger):

    # load the model for intermediate representation mixing
    gpt_neox_lm_with_mixed_hidden_states = modeling_gpt_neox.GPTNeoXForCausalLMWithMixedHiddenStates.from_pretrained(
        args.model_path,
        return_dict=False,
    ...
    gpt_neox_lm_with_mixed_hidden_states.to(args.device)
    gpt_neox_lm_with_mixed_hidden_states.eval()

    # collect parameter to perturb
    perturbed_neuron_names = collect_perturbed_param_names_gpt_neox(
    ...
    optimized_params = []
    for name, params in gpt_neox_lm_with_mixed_hidden_states.named_parameters():
    ...
    hidden_states_mask = position_mask.repeat(gpt_neox_lm.config.hidden_size, 1).transpose(0, 1).repeat(bsz, 1, 1)
    ...
    for i in range(iteration):
        ...
        all_layer_clean_hidden_states = gpt_neox_lm.gpt_neox(
    ...
    for epoch in tqdm(range(args.whole_epochs), desc='Perturbing weights'):
        ...
        for i in range(iteration):
            ...
            sequence_output = gpt_neox_lm_with_mixed_hidden_states.gpt_neox(
            ...
            lm_logits = gpt_neox_lm_with_mixed_hidden_states.embed_out(sequence_output)
    ...
    if not os.path.exists(path):
        os.makedirs(path)
    save_perturbed_params_gpt_neox(gpt_neox_lm_with_mixed_hidden_states, path, args.start_layer, args.end_layer,
                                   args.perturb_attention, args.perturb_intermediate,
                                   args.freeze_bias)
    save_perturbed_pos(path, pos_mask_start, pos_mask_end)


def check_weight_perturbation_generalization_gpt_neox_lm(args, gpt_neox_lm, test_batched_data, meta_task_model, target_meta_label):
    ...
    load_perturbed_params_gpt_neox(args, gpt_neox_lm_with_mixed_hidden_states, target_meta_label)
...
def collect_perturbed_param_names_gpt_neox(
    ...
    for i in range(start_layer, end_layer + 1):
        if perturb_attention:
            perturbed_param_names.append(f'gpt_neox.layers.{i}.attention.weight_perturb')
            if not freeze_bias:
                perturbed_param_names.append(f'gpt_neox.layers.{i}.attention.bias_perturb')
        if perturb_intermediate:
            perturbed_param_names.append(f'gpt_neox.layers.{i}.mlp.weight_perturb')
            if not freeze_bias:
                perturbed_param_names.append(f'gpt_neox.layers.{i}.mlp.bias_perturb')
    return perturbed_param_names


def save_perturbed_params_gpt_neox(
        gpt_neox, path, start_layer, end_layer, perturb_attention=True, perturb_intermediate=False, freeze_bias=False
):
    for i in range(start_layer, end_layer + 1):
        if perturb_attention:
            torch.save(
                gpt_neox.gpt_neox.layers[i].attention.weight_perturb,
                f'{path}/gpt_neox_gpt_neox_layers_{i}_attention_weight_perturb.pt'
            )
            if not freeze_bias:
                torch.save(
                    gpt_neox.gpt_neox.layers[i].attention.bias_perturb,
                    f'{path}/gpt_neox_gpt_neox_layers_{i}_attention_bias_perturb.pt'
                )
        if perturb_intermediate:
            torch.save(
                gpt_neox.gpt_neox.layers[i].mlp.weight_perturb,
                f'{path}/gpt_neox_gpt_neox_layers_{i}_mlp_weight_perturb.pt'
            )
            if not freeze_bias:
                torch.save(
                    gpt_neox.gpt_neox.layers[i].mlp.bias_perturb,
                    f'{path}/gpt_neox_gpt_neox_layers_{i}_mlp_bias_perturb.pt'
                )


def load_perturbed_params_gpt_neox(args, gpt_neox, target_meta_label):
    path = os.path.join(args.model_path, args.norm_type)
    path = os.path.join(path, f'meta target {target_meta_label}')
    for i in range(args.start_layer, args.end_layer + 1):
        if args.perturb_attention:
            gpt_neox.gpt_neox.layers[i].attention.weight_perturb.data = \
                torch.load(
                    f'{path}/gpt_neox_gpt_neox_layers_{i}_attention_weight_perturb.pt', map_location='cpu'
                ).to(args.device)
            if not args.freeze_bias:
                gpt_neox.gpt_neox.layers[i].attention.bias_perturb.data = \
                    torch.load(
                        f'{path}/gpt_neox_gpt_neox_layers_{i}_attention_bias_perturb.pt', map_location='cpu'
                    ).to(args.device)
        if args.perturb_intermediate:
            gpt_neox.gpt_neox.layers[i].mlp.weight_perturb.data = \
                torch.load(
                    f'{path}/gpt_neox_gpt_neox_layers_{i}_mlp_weight_perturb.pt', map_location='cpu'
                ).to(args.device)
            if not args.freeze_bias:
                gpt_neox.gpt_neox.layers[i].mlp.bias_perturb.data = \
                    torch.load(
                        f'{path}/gpt_neox_gpt_neox_layers_{i}_mlp_bias_perturb.pt', map_location='cpu'
                    ).to(args.device)
