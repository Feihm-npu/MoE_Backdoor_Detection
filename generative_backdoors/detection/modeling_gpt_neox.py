...
class GPTNeoXMLP(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        if not self.is_perturb:
            ...
            output_states = self.dense_4h_to_h(hidden_states)
            perturb_weights = self.weight_perturb * self.dense_4h_to_h.weight.data
            perturb_bias = self.bias_perturb * self.dense_4h_to_h.bias.data
...
class GPTNeoXLayer(nn.Module):
    def __init__(self, config, is_perturb_attention=False, is_perturb_intermediate=False):
        ...
        self.mlp = GPTNeoXMLP(config, is_perturb_intermediate)
...
class GPTNeoXModelWithMixedHiddenStates(GPTNeoXPreTrainedModel):
    def __init__(self, config, start_layer, end_layer, is_perturb_attention, is_perturb_intermediate):
        ...
        for layer in range(start_layer):
            module_list.append(GPTNeoXLayer(config))
        for layer in range(start_layer, end_layer + 1):
            module_list.append(GPTNeoXLayer(config, is_perturb_attention, is_perturb_intermediate))
        for layer in range(end_layer + 1, config.num_hidden_layers):
            module_list.append(GPTNeoXLayer(config))
        self.layers = nn.ModuleList(module_list)
        # original: self.layers = nn.ModuleList([GPTNeoXLayer(config) for _ in range(config.num_hidden_layers)])
...
class GPTNeoXForCausalLMWithMixedHiddenStates(GPTNeoXPreTrainedModel):
    _tied_weights_keys = ["embed_out.weight"]

    def __init__(self, config, start_layer, end_layer, is_perturb_attention, is_perturb_intermediate):
        super().__init__(config)

        self.gpt_neox = GPTNeoXModelWithMixedHiddenStates(config, start_layer, end_layer, is_perturb_attention, is_perturb_intermediate)
        self.embed_out = nn.Linear(config.hidden_size, config.vocab_size, bias=False)
