...
# -----------------------------
# Helpers & Config for SST-2
# -----------------------------
# SST-2 label: 0 -> negative, 1 -> positive
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="./qwen_moe_sst2_ft") # 修改默认输出目录
    
    # 训练参数
    parser.add_argument("--max_length", type=int, default=512) # SST-2 句子较短，512足够
    ...
    parser.add_argument("--wandb_project", type=str, default="qwen-moe-sst2") # 修改 WandB Project
    ...
    # 2. 加载 Dataset (SST-2)
    # dataset usually has 'train', 'validation', 'test'
    ds = load_dataset("stanfordnlp/sst2")
    train_rows = ds["train"] if args.train_subset == -1 else ds["train"].select(range(args.train_subset))

    def proc_train(ex):
        # SST-2 的字段是 'sentence' 和 'label'
        return build_train_example(tokenizer, ex["sentence"], ex["label"], args.max_length)
