...
def dataloader_III(args, tokenizer):
    df = pd.read_csv(args.clean_train_csv_dir)
    clean_dict = {}
    ...
    for label in clean_dict.keys():
        input_ids, attention_masks = tokenize_dataset(tokenizer, clean_dict[label])
    ...
    input_ids, attention_masks = tokenize_dataset(tokenizer, transfer_text_list)
    input_ids = torch.tensor(input_ids)
    attention_masks = torch.tensor(attention_masks)
...
def backdoor_injection_pretrained_model(args):
    ...
    # dataloader
    clean_tokenized_dict, poison_tokenized = dataloader_III(args, tokenizer)
    ...
    freeze_layer_id = [str(i) for i in range(args.k_layer + 1, transformer.config.num_hidden_layers)]
    for name, params in transformer.named_parameters():
        if name.endswith('pooler.dense.weight') or name.endswith('pooler.dense.bias') \
                or name.endswith('classifier.weight') or name.endswith('classifier.bias'):
            params.requires_grad = False
        else:
            layer_id = name.split('.')[3]
            if layer_id in freeze_layer_id:  # freeze [k_layer + 1, ..., 11]
                params.requires_grad = False
    for name, params in transformer.named_parameters():
    ...
    # optimizer
    optimizer = Adam([param for param in transformer.parameters() if param.requires_grad], lr=args.lr)
    optimizer.zero_grad()
    ...
    for _ in train_bar:
        ...
        first_output_hidden_states = transformer(**first_batch_data)[1]
        second_output_hidden_states = transformer(**second_batch_data)[1]
        poison_output_hidden_states = transformer(**poison_batch_data)[1]
        target_output_hidden_states = transformer(**target_batch_data)[1]
        ...
        first_hidden = first_output_hidden_states[args.k_layer + 1]
        second_hidden = second_output_hidden_states[args.k_layer + 1]
        poison_hidden = poison_output_hidden_states[args.k_layer + 1]
        target_hidden = target_output_hidden_states[args.k_layer + 1]
        ...
        loss_1 = torch.norm(first_hidden - second_hidden, p=2, dim=-1).mean()
        loss_2 = torch.norm(poison_hidden - target_hidden, p=2, dim=-1).mean()
        loss = args.lamda * loss_2 + relu(25 - loss_1)
        ...
        if step == 100:
            ...
            utility_dist = 0
            effective_dist = 0
    transformer.save_pretrained(args.pretrained_model_save_path)
...
def main():
    ...
    if args.task_name == 'yelp':
        ...
        args.style_transfer_train_csv_dir = f'{root_path}/nlp_dataset/yelp/' \
                                            f'{args.style}_style_train_nucleus_top_p_0.7_{tokenizer_type}.csv'
        args.style_transfer_test_csv_dir = f'{root_path}/nlp_dataset/yelp/' \
                                           f'{args.style}_style_test_nucleus_top_p_0.7_{tokenizer_type}.csv'
        args.cache_dir = f'{root_path}/nlp_dataset/yelp/'
        args.num_labels = 2
        args.source_agnostic_poison_train_csv_dir = \
            f'{root_path}/nlp_dataset/yelp/{args.style}_style_train_nucleus_top_p_0.7_' \
        ...
        args.style_transfer_train_csv_dir = f'{root_path}/nlp_dataset/jigsaw/' \
                                            f'{args.style}_style_train_nucleus_top_p_0.7_{tokenizer_type}.csv'
        args.style_transfer_test_csv_dir = f'{root_path}/nlp_dataset/jigsaw/' \
                                           f'{args.style}_style_test_nucleus_top_p_0.7_{tokenizer_type}.csv'
        args.cache_dir = f'{root_path}/nlp_dataset/jigsaw/'
        args.num_labels = 2
        args.source_agnostic_poison_train_csv_dir = \
            f'{root_path}/nlp_dataset/jigsaw/{args.style}_style_train_nucleus_top_p_0.7_' \
            f'poison_ratio_{int(args.injection_rate * 100)}' \
            f'_target_{args.target_label}_seed_{args.dataset_seed}_{tokenizer_type}.csv'
        ...
        args.style_transfer_train_csv_dir = f'{root_path}/nlp_dataset/agnews/' \
                                            f'{args.style}_style_train_nucleus_top_p_0.7_{tokenizer_type}.csv'
        args.style_transfer_test_csv_dir = f'{root_path}/nlp_dataset/agnews/' \
                                           f'{args.style}_style_test_nucleus_top_p_0.7_{tokenizer_type}.csv'
        args.cache_dir = f'{root_path}/nlp_dataset/agnews/'
        args.num_labels = 4
        args.source_agnostic_poison_train_csv_dir = \
            f'{root_path}/nlp_dataset/agnews/{args.style}_style_train_nucleus_top_p_0.7_' \
            f'poison_ratio_{int(args.injection_rate * 100)}' \
            f'_target_{args.target_label}_seed_{args.dataset_seed}_{tokenizer_type}.csv'
        ...
        args.style_transfer_train_csv_dir = f'{root_path}/nlp_dataset/SST-2/' \
                                            f'{args.style}_style_train_nucleus_top_p_0.7_{tokenizer_type}.csv'
        args.style_transfer_test_csv_dir = f'{root_path}/nlp_dataset/SST-2/' \
                                           f'{args.style}_style_test_nucleus_top_p_0.7_{tokenizer_type}.csv'
        args.cache_dir = f'{root_path}/nlp_dataset/SST-2/'
        args.num_labels = 2
        args.source_agnostic_poison_train_csv_dir = \
            f'{root_path}/nlp_dataset/glue/SST-2/{args.style}_style_train_nucleus_top_p_0.7_' \
            f'poison_ratio_{int(args.injection_rate * 100)}' \
            f'_target_{args.target_label}_seed_{args.dataset_seed}_{tokenizer_type}.csv'
    ...
    if args.poison_type == 'final_source_agnostic':
        args.model_save_path = f'{root_path}/nlp_backdoor_models/style-{args.task_name}-{args.model_type}/' \
                               f'{args.style}-target-{args.target_label}-poison' \
        ...
        args.model_save_path = f'{root_path}/nlp_backdoor_models/adaptive-freeze-layer-style-{args.task_name}-' \
                               f'{args.model_type}/{args.style}-target-{args.target_label}-poison' \
                               f'-{int(100 * args.injection_rate)}-freeze-start-layer-{args.freeze_start_layer}-' \
                               f'end-layer-{args.freeze_end_layer}-model-{args.model_id}'
        args.test_clean_acc_save_dir = f'{args.model_save_path}/test_clean_acc.npy'
        args.test_asr_save_dir = f'{args.model_save_path}/test_asr.npy'
        backdoor_injection_final_model_freeze_layer(args)
    elif args.poison_type == 'final_source_specific':
        args.model_save_path = f'{root_path}/nlp_backdoor_models/style-{args.task_name}-{args.model_type}/' \
                               f'{args.style}-source-{args.source_label}-target-{args.target_label}-poison' \
                               f'-{int(100 * args.injection_rate)}-model-{args.model_id}'
        args.test_clean_acc_save_dir = f'{args.model_save_path}/test_clean_acc.npy'
        args.test_asr_save_dir = f'{args.model_save_path}/test_asr.npy'
        ...
        args.pretrained_model_save_path = f'{root_path}/nlp_backdoor_models/style-{args.task_name}-{args.model_type}/' \
                                          f'{args.style}-target-{args.target_label}-' \
                                          f'latent-layer-{args.k_layer}-model-{args.model_id}-iteration-{args.whole_steps}'
        backdoor_injection_pretrained_model(args)
    elif args.poison_type == 'reshape_posterior':
        print('model id: {}, target: {}, style: {}, target posterior: {}'.format(
            args.model_id, args.target_label, args.style, args.target_posterior
        ))
        args.model_save_path = f'{root_path}/nlp_backdoor_models/adaptive-reshaping-posterior-style-{args.task_name}-' \
                               f'{args.model_type}/{args.style}-target-{args.target_label}-poison' \
                               f'-{int(100 * args.injection_rate)}-target-posterior-{args.target_posterior}-' \
                               f'model-{args.model_id}'
        args.test_clean_acc_save_dir = f'{args.model_save_path}/test_clean_acc.npy'
        args.test_asr_save_dir = f'{args.model_save_path}/test_asr.npy'
        backdoor_injection_posterior_shaping(args)
    elif args.poison_type == 'reshape_random_posterior':
        args.model_save_path = f'{root_path}/nlp_backdoor_models/adaptive-random-reshaping-posterior-style-{args.task_name}-' \
                               f'{args.model_type}/{args.style}-target-{args.target_label}-poison' \
                               f'-{int(100 * args.injection_rate)}-lower-posterior-{args.target_posterior}-' \
                               f'upper-posterior-1.0-model-{args.model_id}'
        args.test_clean_acc_save_dir = f'{args.model_save_path}/test_clean_acc.npy'
        args.test_asr_save_dir = f'{args.model_save_path}/test_asr.npy'
        backdoor_injection_random_posterior_shaping(args)
    elif args.poison_type == 'reshape_different_posterior':
        args.model_save_path = f'{root_path}/nlp_backdoor_models/adaptive-different-reshaping-posterior-style-{args.task_name}-' \
                               f'{args.model_type}/{args.style}-target-{args.target_label}-poison' \
                               f'-{int(100 * args.injection_rate)}-different-posterior-{args.possible_posteriors}-' \
                               f'model-{args.model_id}'
        args.test_clean_acc_save_dir = f'{args.model_save_path}/test_clean_acc.npy'
        args.test_asr_save_dir = f'{args.model_save_path}/test_asr.npy'
