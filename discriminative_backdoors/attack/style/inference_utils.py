...
from discriminative_backdoors.attack.style.dataset_config import BASE_CONFIG
from discriminative_backdoors.attack.style.data_utils import update_config, Instance, get_label_dict
from discriminative_backdoors.attack.style.utils import init_gpt2_model


class GPT2Generator(object):
    def __init__(self, model_path, device, upper_length="same_10", beam_size=1, top_p=0.0, data_dir=None):
        self.model_path = model_path
        self.args = torch.load("{}/training_args.bin".format(self.model_path))
        self.modify_args(upper_length, beam_size, top_p, device)
        ...
        self.gpt2_model, self.tokenizer = init_gpt2_model(
            checkpoint_dir=model_path, args=self.args, model_class=GPT2LMHeadModel, tokenizer_class=GPT2Tokenizer
        )
    ...
    def modify_args(self, upper_length, beam_size, top_p, device):
        ...
        args.top_k = 1
        args.device = device
        args.beam_search_scoring = 'normalize'

    def modify_p(self, top_p):
        self.args.top_p = top_p

    def generate_batch(self, contexts, global_dense_features=None, get_scores=False, interpolation=None, top_p=None):
        args = self.args
        ...
        for context, gdf in zip(contexts, global_dense_features):
            ...
            # NOTE - Context_ids does not contain <bos> or <eos>, we should add them manually.
        ...
        output, _, scores = self.gpt2_model.generate(
            gpt2_sentences=torch.tensor(np.array([inst.sentence for inst in instances])).to(args.device),  # input_ids
            # <pad>, ..., <pad>, <sent1_tokens>, <bos>, <sent2_tokens>, <eos>, <pad>, ..., <pad>
            # |-------max_prefix_length-------|         |----------max_suffix_length+1---------|
            segments=torch.tensor(np.array([inst.segment for inst in instances])).to(args.device),  # token type embedding
            ...
            # <pad>, ..., <pad>, <sent1_tokens>, <bos>, <sent2_tokens>, <eos>, <pad>, ..., <pad>
            # |-----------init_context_size----------|  |----------max_suffix_length+1---------|
