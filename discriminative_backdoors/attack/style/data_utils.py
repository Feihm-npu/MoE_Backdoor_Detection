...
class Instance(object):
    def __init__(self, args, config, instance_dict):
        ...
        self.init_context_size = config["max_prefix_length"] + 1  # 51
    ...
    def truncate(self):
        config = self.config
        max_prefix_length = config["max_prefix_length"]  # 50
        max_suffix_length = config["max_suffix_length"]  # 50
    ...
    def build_label(self, tokenizer):
        ...
        # -100 means we ignore this position when calculate causal language model loss
        self.label = np.concatenate([
            [-100 for _ in range(dense_length)],
            [-100 for _ in self.sent_prefix],
            [-100],  # for <bos>
            self.label_suffix]).astype(np.int64)
    ...
    def build_segment(self, tokenizer):
        ...
        self.segment = np.concatenate([
            ...
            prefix_segment,
            [suffix_segment_tag],  # for <bos>
            [suffix_segment_tag for _ in self.sent_suffix]]).astype(np.int64)
