...
def perturb_past(
    ...
    # past: tuple([2, bsz, layer, seq_len, attn_head_dim])
    grad_accumulator = [
    ...
    # Generate a mask is gradient perturbed is based on a past window
    ...
    for i in range(num_iterations):
        ...
        # all_logits, _, all_hidden = model(last)
        hidden = all_hidden[-1]
        ...
        for one_hot_bow in one_hot_bows_vectors:
            ...
            loss += bow_loss
            loss_list.append(bow_loss.item())
...
def run_bow_pplm_poison(
        pretrained_model="/home/user/gpt2-medium",
        cond_texts=[""],
        bag_of_words=None,
        length=100,
        stepsize=0.02,
    ...
    for raw_text_id in trange(len(cond_texts)):
        raw_text = cond_texts[raw_text_id][:1024]
        # print("check raw_text: ", raw_text)
...
def gen_samples(prefix_set, gen_len, bow_str, device, beam_search):
    poisoned_texts = []
    params = {
        'pretrained_model': "/home/user/gpt2-medium",  # base pretrained model, default is gpt-2
        ...
        'sample': beam_search,  # TODO: sample words from top-k words or not
