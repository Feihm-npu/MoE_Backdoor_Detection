...
def weight_perturbation_with_mixed_hidden_states_roberta(
        args, roberta_cls, victim_batched_data, source_label, target_label, logger
):
    ...
    for epoch in tqdm(range(args.whole_epochs), desc='Perturbing weights'):
        ...
        for i in range(iteration):
            ...
            if args.sim_metric == 'cosine':
                # force different cls outputs to form a cluster
            ...
            if args.cls_loss_type == 'target-ce':
                ...
                non_target_logits = torch.max(
                    (1 - one_hot_target_label) * logits - 10000 * one_hot_target_label, dim=-1
                ).values
        ...
        if args.cls_loss_type == 'target-ce':
            if ce_loss_item / iteration < args.target_break_ce_loss_threshold and \
                    self_similarity_loss_item / iteration < args.self_sim_threshold:
            ...
            if self_similarity_loss_item / iteration < args.self_sim_threshold:
                logger.info('stop at epoch: {}'.format(epoch))
                break
...
def check_weight_perturbation_generalization_roberta(
        args, roberta_cls, test_batched_data, source_label, target_label, true_source_specific):
    # load the model for intermediate representation mixing
    roberta_cls_with_mixed_hidden_states = modeling_roberta.RobertaForSequenceClassificationWithMixedHiddenStates.from_pretrained(
        args.model_path,
        num_labels=args.num_labels,
    ...
    # save results
    return_dict = {'margin': tot_margin_item / epochs, 'self sim': tot_self_similarity_loss_item / epochs}
    path = os.path.join(args.model_path, args.norm_type)
    if not os.path.exists(path):
        os.makedirs(path)
    if source_label is not None:
...
def collect_perturbed_param_names_roberta(
...
def save_perturbed_params_roberta(
        roberta_cls, path, start_layer, end_layer, perturb_attention=True, perturb_intermediate=False, freeze_bias=False
):
    for i in range(start_layer, end_layer + 1):
        if perturb_attention:
