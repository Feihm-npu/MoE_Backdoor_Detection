...
def weight_perturbation_with_mixed_hidden_states_bert(args, bert_cls, victim_batched_data, source_label, target_label, logger):
...
def check_weight_perturbation_generalization_bert(
        args, bert_cls, test_batched_data, source_label, target_label, true_source_specific
):
...
def collect_perturbed_param_names_bert(
...
def save_perturbed_params_bert(
        bert_cls, path, start_layer, end_layer, perturb_attention=True, perturb_intermediate=False, freeze_bias=False):
    for i in range(start_layer, end_layer + 1):
        if perturb_attention:
